...
/**
 * The base condition-bean of RECORD.
 * @author DBFlute(AutoGenerator)
 */
public class BsRecordCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected RecordCQ _conditionQuery;
     ...
     * @return this. (NotNull)
     */
    public RecordCB acceptPK(Long recordId) {
        assertObjectNotNull("recordId", recordId);
        BsRecordCB cb = this;
        cb.query().setRecordId_Equal(recordId);
        return (RecordCB)this;
    }

    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_RecordId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_RecordId_Desc();
        return this;
    }
     ...
     */
    public RecordCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public RecordCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected RecordCQ doGetConditionQuery() {
    ...
    protected RecordCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected RecordCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        RecordCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected RecordCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new RecordCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<RecordCB> unionCBLambda) {
        final RecordCB cb = new RecordCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RecordCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<RecordCB> unionCBLambda) {
        final RecordCB cb = new RecordCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RecordCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }

    // ===================================================================================
    //                                                                         SetupSelect
    //                                                                         ===========
    protected AccountItemNss _nssAccountItem;
    public AccountItemNss xdfgetNssAccountItem() {
        if (_nssAccountItem == null) { _nssAccountItem = new AccountItemNss(null); }
     ...
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_AccountItem()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     ...
     *     ... = <span style="color: #553000">record</span>.<span style="color: #CC4747">getAccountItem()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    public AccountItemNss setupSelect_AccountItem() {
        assertSetupSelectPurpose("accountItem");
        if (hasSpecifiedLocalColumn()) {
            specify().columnAccountItemId();
        }
        doSetupSelect(() -> query().queryAccountItem());
        if (_nssAccountItem == null || !_nssAccountItem.hasConditionQuery())
        { _nssAccountItem = new AccountItemNss(query().queryAccountItem()); }
        return _nssAccountItem;
    }
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">record</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">record</span>.<span style="color: #CC4747">getMember()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<RecordCQ> {
         ...
         */
        public SpecifiedColumn columnRecordId() { return doColumn("RECORD_ID"); }
        /**
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnDepositAccountId() { return doColumn("DEPOSIT_ACCOUNT_ID"); }
        /**
         * (出金元口座ID)WITHDRAWAL_ACCOUNT_ID: {IX, BIGINT(19), FK to ASSET}
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnWithdrawalAccountId() { return doColumn("WITHDRAWAL_ACCOUNT_ID"); }
        /**
        ...
        protected void doSpecifyRequiredColumn() {
            columnRecordId(); // PK
            if (qyCall().qy().hasConditionQueryAccountItem()
        ...
        @Override
        protected String getTableDbName() { return "RECORD"; }
        /**
         * Prepare to specify functions about relation table. <br>
         * (勘定科目)ACCOUNT_ITEM by my ACCOUNT_ITEM_ID, named 'accountItem'.
         * @return The instance for specification for relation table to specify. (NotNull)
         */
        public AccountItemCB.HpSpecification specifyAccountItem() {
            assertRelation("accountItem");
            if (_accountItem == null) {
                _accountItem = new AccountItemCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryAccountItem()
                                    , () -> _qyCall.qy().queryAccountItem())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _accountItem.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryAccountItem()
                      , () -> xsyncQyCall().qy().queryAccountItem()));
                }
            }
            return _accountItem;
        }
         ...
         */
        public HpSDRFunction<RecordCB, RecordCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<RecordCB> sq, RecordCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public RecordCB dreamCruiseCB() {
        RecordCB cb = new RecordCB();
        cb.xsetupForDreamCruise((RecordCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<RecordCB> columnQuery(final SpecifyQuery<RecordCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected RecordCB xcreateColumnQueryCB() {
        RecordCB cb = new RecordCB();
        cb.xsetupForColumnQuery((RecordCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<RecordCB> orCBLambda) {
        xorSQ((RecordCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<RecordCB> andCBLambda) {
        xorSQAP((RecordCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return RecordCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return RecordCQ.class.getName(); }
