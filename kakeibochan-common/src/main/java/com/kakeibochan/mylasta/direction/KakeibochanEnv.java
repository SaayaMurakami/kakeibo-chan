 ...
 * @author FreeGen
 */
public interface KakeibochanEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/kakeibochan */
    ...
    /** The key of the configuration. e.g. returnpath@kakeibochan.com */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. com.mysql.jdbc.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:mysql://localhost:3306/kakeibochandb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. kakeibochandb */
     ...
     * The value is, e.g. /tmp/lastaflute/kakeibochan <br>
     ...
     * The value is, e.g. returnpath@kakeibochan.com <br>
     ...
     * The value is, e.g. jdbc:mysql://localhost:3306/kakeibochandb <br>
     ...
     * The value is, e.g. kakeibochandb <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements KakeibochanEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(KakeibochanEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(KakeibochanEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(KakeibochanEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(KakeibochanEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(KakeibochanEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(KakeibochanEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(KakeibochanEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(KakeibochanEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(KakeibochanEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(KakeibochanEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(KakeibochanEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(KakeibochanEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(KakeibochanEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(KakeibochanEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(KakeibochanEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(KakeibochanEnv.MAIL_RETURN_PATH);
        }

        public String getJdbcDriver() {
            return get(KakeibochanEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(KakeibochanEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(KakeibochanEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(KakeibochanEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(KakeibochanEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(KakeibochanEnv.JDBC_CONNECTION_POOLING_SIZE);
        }
    }
}
